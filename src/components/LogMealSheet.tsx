...
import { Camera, ChevronLeft, X, Check, Upload } from "lucide-react";
import { calculateMacros, MOCK_FOOD_NAME, FoodEntry } from "@/types/food";

type Step = "scan" | "grams" | "summary" | "logging";
...
const LogMealSheet: React.FC<LogMealSheetProps> = ({ open, onClose, onLog }) => {
  ...
  const macros = calculateMacros(gramsNum);
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImagePreview(url);
    // Simulate scan delay then advance
    setTimeout(() => setStep("grams"), 600);
  };
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3">
          ...
          <div className="flex gap-1.5">
            {(["scan", "grams", "summary"] as Step[]).map((s, i) => (
              <div
                key={s}
                className="h-1 rounded-full transition-all duration-400"
                style={{
                  width: step === s ? 20 : 6,
                  background: step === s ? "#f74db9" : "rgba(255,255,255,0.2)",
                }}
              />
            ))}
          </div>
